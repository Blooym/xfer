...
impl Cryptography {
    ...
    /// Encrypt `reader` and write the output to `writer`.
    ///
    ...
    ///
    /// Note that this method will seek the reader and writer to the beginning
    /// at the start and end of the encryption process.
    pub fn encrypt<R: Read + Seek, W: Write + Seek>(
        reader: &mut R,
        writer: &mut W,
    ) -> Result<String> {
        ...
        // Reset to the start of the stream for
        // any future usage.
        reader.seek(std::io::SeekFrom::Start(0))?;
        writer.seek(std::io::SeekFrom::Start(0))?;
        Ok(passphrase)
    ...
    /// Decrypt `reader` and write the output `writer`.
    ///
    ...
    pub fn decrypt<R: Read + Seek, W: Write + Seek>(
        passphrase: &str,
        reader: &mut R,
        writer: &mut W,
    ) -> Result<(), anyhow::Error> {
